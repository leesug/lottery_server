-- 수정된 데이터베이스 이전 SQL 스크립트 (충돌 해결)
-- lotto_system에서 lotto_server로 데이터 이전

-- ---------------------------------------------------
-- 사용자 데이터 이전 (충돌 제외)
-- ---------------------------------------------------
-- 현재 lotto_server.users 테이블에 없는 사용자만 추가
...
(log_type, message, source, ip_address, user_id, created_at)
SELECT log_type, message, source, ip_address, user_id, created_at 
...
-- ---------------------------------------------------
-- terminals 및 tickets 테이블 데이터는 수동 검토 후 이전 권장
-- ---------------------------------------------------
-- 두 테이블의 구조가 크게 다를 수 있으므로 수동 검토 후 이전이 권장됩니다.
