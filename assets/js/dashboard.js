...
function loadTabContent(tabId, contentId) {
    ...
    // 로드할 대시보드 경로 결정
    let dashboardPath = getDashboardPath(tabId);
    if (!dashboardPath) {
        console.error('대시보드 경로를 찾을 수 없습니다: ' + tabId);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#' + contentId).html(response).data('loaded', true);
 ...
 * 탭 ID에 따른 대시보드 경로 반환
 * @param {string} tabId - 활성화된 탭의 ID
 * @return {string|null} 대시보드 경로 또는 null
 */
function getDashboardPath(tabId) {
    // 탭 ID에서 타입 추출 (예: tab-lottery -> lottery)
    const dashboardType = tabId.replace('tab-', '');
    
    // API 엔드포인트를 사용하여 콘텐츠만 로드
    return SERVER_URL + '/dashboard/api/get-dashboard-content.php?type=' + dashboardType;
}
