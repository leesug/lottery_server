...
$query = "
    SELECT 
        ...
        (COALESCE(w1.winner_count, 0) + COALESCE(w2.winner_count, 0) + COALESCE(w3.winner_count, 0) + 
         COALESCE(w4.winner_count, 0) + COALESCE(w5.winner_count, 0)) AS total_winners
    FROM 
        draws d
    LEFT JOIN 
...
if ($prizeLevel > 0) {
    switch ($prizeLevel) {
        case 1:
            $query .= " AND COALESCE(w1.winner_count, 0) > 0";
            break;
        case 2:
            $query .= " AND COALESCE(w2.winner_count, 0) > 0";
            break;
        case 3:
            $query .= " AND COALESCE(w3.winner_count, 0) > 0";
            break;
        case 4:
            $query .= " AND COALESCE(w4.winner_count, 0) > 0";
            break;
        case 5:
            $query .= " AND COALESCE(w5.winner_count, 0) > 0";
            break;
    }
}
...
$stmt->execute();
$winnersData = $stmt->fetchAll(PDO::FETCH_ASSOC);

// 전체 당첨자 합계 (가상 데이터 - 실제로는 DB에서 가져와야 함)
$totalPrize1Winners = 0;
$totalPrize2Winners = 0;
$totalPrize3Winners = 0;
$totalPrize4Winners = 0;
$totalPrize5Winners = 0;
$totalAllWinners = 0;

foreach ($winnersData as $row) {
    $totalPrize1Winners += $row['prize1_winners'];
    $totalPrize2Winners += $row['prize2_winners'];
    $totalPrize3Winners += $row['prize3_winners'];
    $totalPrize4Winners += $row['prize4_winners'];
    $totalPrize5Winners += $row['prize5_winners'];
    $totalAllWinners += $row['total_winners'];
}
...
<section class="content">
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-lg-3 col-6">
                ...
                <div class="small-box bg-info">
                    <div class="inner">
                        <h3><?php echo number_format($totalPrize1Winners); ?>명</h3>
                        <p>1등 당첨자 수</p>
                    </div>
                    <div class="icon">
            ...
            <div class="col-lg-3 col-6">
                ...
                <div class="small-box bg-success">
                    <div class="inner">
                        <h3><?php echo number_format($totalPrize2Winners); ?>명</h3>
                        <p>2등 당첨자 수</p>
                    </div>
                    <div class="icon">
            ...
            <div class="col-lg-3 col-6">
                ...
                <div class="small-box bg-warning">
                    <div class="inner">
                        <h3><?php echo number_format($totalAllWinners); ?>명</h3>
                        <p>전체 당첨자 수</p>
                    </div>
                    <div class="icon">
        ...
        <div class="card">
            ...
            <!-- /.card-header -->
            <div class="card-body">
                <form id="winnersFilterForm" method="GET" action="">
                    <div class="row">
                        <div class="col-md-2">
                    ...
                    <div class="row">
                        <div class="col-md-12 text-right">
                            ...
                            <button type="button" class="btn btn-default" onclick="window.location.href='winners.php'">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">회차별 당첨자 현황</h3>
                <div class="card-tools">
                    <button type="button" class="btn btn-tool" data-card-widget="collapse">
                        <i class="fas fa-minus"></i>
                    </button>
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                    ...
                    <tbody>
                        <?php if (empty($winnersData)): ?>
                            <tr>
                                <td colspan="10" class="text-center">데이터가 없습니다.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($winnersData as $row): ?>
                                <tr>
                                    ...
                                    <td><?php echo number_format($row['prize2_winners']); ?>명</td>
                                    <td><?php echo number_format($row['prize3_winners']); ?>명</td>
                                    <td><?php echo number_format($row['prize4_winners']); ?>명</td>
                                    <td><?php echo number_format($row['prize5_winners']); ?>명</td>
                                    <td><?php echo number_format($row['total_winners']); ?>명</td>
                                    <td>
                                        <button type="button" class="btn btn-sm btn-info" onclick="showWinnersDetails(<?php echo $row['draw_id']; ?>)">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">당첨자 분포 및 추이</h3>
                <div class="card-tools">
                    <button type="button" class="btn btn-tool" data-card-widget="collapse">
                        <i class="fas fa-minus"></i>
                    </button>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="chart">
                            <canvas id="winnersDistributionChart" style="min-height: 300px; height: 300px; max-height: 300px; max-width: 100%;"></canvas>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="chart">
                            <canvas id="winnersTrendChart" style="min-height: 300px; height: 300px; max-height: 300px; max-width: 100%;"></canvas>
                        </div>
                    </div>
                </div>
            </div>
...
<!-- 당첨자 상세 정보 모달 -->
<div class="modal fade" id="winnersDetailsModal">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">당첨자 상세 정보</h4>
            ...
            <div class="modal-footer justify-content-between">
                ...
                <button type="button" class="btn btn-primary" onclick="printWinnersDetails()">인쇄</button>
            </div>
        </div>
        <!-- /.modal-content -->
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const distributionData = [
        <?php echo $totalPrize1Winners; ?>,
        <?php echo $totalPrize2Winners; ?>,
        <?php echo $totalPrize3Winners; ?>,
        <?php echo $totalPrize4Winners; ?>,
        <?php echo $totalPrize5Winners; ?>
    ];
    
    // 당첨자 분포 차트 생성
    var distributionChartCanvas = document.getElementById('winnersDistributionChart').getContext('2d');
    ...
    $chartData = array_slice($winnersData, 0, 6);
    $chartData = array_reverse($chartData);
    foreach ($chartData as $row): 
    ?>
        drawNumbers.push('<?php echo $row['draw_code']; ?>회');
        prize1Data.push(<?php echo $row['prize1_winners']; ?>);
        prize2Data.push(<?php echo $row['prize2_winners']; ?>);
        prize3Data.push(<?php echo $row['prize3_winners']; ?>);
    <?php endforeach; ?>
    
    // 당첨자 추이 차트 생성
    var trendChartCanvas = document.getElementById('winnersTrendChart').getContext('2d');
    var trendChartData = {
        labels: drawNumbers,
        datasets: [
            {
...
// 당첨자 상세 정보 표시
function showWinnersDetails(drawId) {
    // AJAX로 상세 정보 가져오기
    $('#winnersDetailsModal').modal('show');
    
    // 여기에서 AJAX 요청을 통해 상세 정보를 가져옵니다.
    // 실제 구현시에는 아래 코드를 사용하세요.
    /*
    $.ajax({
        url: 'get_winners_details.php',
        type: 'GET',
        data: {
            draw_id: drawId
        },
        success: function(response) {
            $('#winnersDetailsContent').html(response);
        },
        error: function() {
            $('#winnersDetailsContent').html('<div class="alert alert-danger">데이터를 불러오는 데 실패했습니다.</div>');
        }
    });
    */
                ...
                <div class="col-md-12">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <th>총 당첨자수</th>
                            <td id="detailTotalWinners">250명</td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="chart-responsive">
                        <canvas id="detailWinnersChart" height="200"></canvas>
                    </div>
                </div>
            </div>
        `);
...
function drawDetailCharts() {
    // 지역별 당첨자 분포 차트
    var regionChartCanvas = document.getElementById('detailWinnersChart').getContext('2d');
    var regionChartData = {
        labels: ['서울', '경기', '대전'],
        datasets: [
            {
...
// 당첨자 상세 정보 인쇄
function printWinnersDetails() {
    var printContents = document.getElementById('winnersDetailsContent').innerHTML;
    var originalContents = document.body.innerHTML;
    
    document.body.innerHTML = `
        <div class="container mt-3">
            <h2 class="text-center mb-4">당첨자 상세 정보</h2>
            ${printContents}
        </div>
    `;
...
function exportToExcel() {
    // 여기에 엑셀 다운로드 기능 구현
    alert('당첨자 데이터를 엑셀로 다운로드합니다.');
    // 실제 구현시에는 아래와 같이 서버에 요청하여 엑셀 파일을 다운로드합니다.
    // window.location.href = 'export_winners.php' + window.location.search;
}
