 ...
 * 재무 관리 - 거래 수정 페이지
 * 
 * 이 페이지는 기존 재무 거래를 수정하는 기능을 제공합니다.
 */
...
if (!$transaction) {
    ...
    $transaction = [
        ...
        'category_id' => $type == 'income' ? 1 : ($type == 'expense' ? 3 : 9),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = sanitizeInput($_POST['status'] ?? ''); // 상태 업데이트 허용
    ...
    if (empty($errors)) {
        try {
            ...
            $sql = "UPDATE financial_transactions SET 
                    ...
                    description = ?, category_id = ?, reference_type = ?, reference_id = ?, 
                    payment_method = ?, payment_details = ?, status = ?, notes = ?, updated_at = NOW()
                    WHERE id = ?";
            
            $stmt = $db->prepare($sql);
            
            $stmt->bindParam(1, $transactionType);
            $stmt->bindParam(2, $amount);
            $stmt->bindParam(3, $currency);
            $stmt->bindParam(4, $transactionDate);
            $stmt->bindParam(5, $description);
            $stmt->bindParam(6, $categoryId);
            $stmt->bindParam(7, $referenceType);
            $stmt->bindParam(8, $referenceId);
            $stmt->bindParam(9, $paymentMethod);
            $stmt->bindParam(10, $paymentDetails);
            $stmt->bindParam(11, $status);
            $stmt->bindParam(12, $notes);
            $stmt->bindParam(13, $transactionId, PDO::PARAM_INT);
...
// 거래일시 포맷 변환 (YYYY-MM-DDTHH:MM 형식으로)
$transactionDateFormatted = date('Y-m-d\TH:i', strtotime($transaction['transaction_date']));
        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">거래 정보 수정</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">거래 정보 수정</h3>
            </div>
            <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="transaction_code">거래 코드</label>
                                <input type="text" class="form-control" id="transaction_code" value="<?php echo htmlspecialchars($transaction['transaction_code']); ?>" readonly>
                                <small class="text-muted">거래 코드는 변경할 수 없습니다.</small>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="number" step="0.01" min="0.01" class="form-control" id="amount" name="amount" placeholder="금액" value="<?php echo htmlspecialchars($transaction['amount']); ?>" required>
                                    <div class="input-group-append">
                                        <select class="form-control" id="currency" name="currency">
                                            <option value="NPR" <?php echo $transaction['currency'] == 'NPR' ? 'selected' : ''; ?>>NPR</option>
                            ...
                            <div class="form-group">
                                ...
                                <input type="datetime-local" class="form-control" id="transaction_date" name="transaction_date" value="<?php echo $transactionDateFormatted; ?>" required>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="payment_method" name="payment_method" required>
                                    ...
                                    <option value="<?php echo $key; ?>" <?php echo $transaction['payment_method'] == $key ? 'selected' : ''; ?>>
                                        <?php echo $value; ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="status" name="status">
                                    ...
                                    <option value="<?php echo $key; ?>" <?php echo $transaction['status'] == $key ? 'selected' : ''; ?>>
                                        <?php echo $value; ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="3" placeholder="거래에 대한 설명"><?php echo htmlspecialchars($transaction['description'] ?? ''); ?></textarea>
                            </div>
                            
                            <div class="form-group">
                                <label for="payment_details">결제 상세 정보</label>
                                <textarea class="form-control" id="payment_details" name="payment_details" rows="2" placeholder="결제 관련 상세 정보"><?php echo htmlspecialchars($transaction['payment_details'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="reference_type" name="reference_type">
                                    ...
                                    <option value="sale" <?php echo $transaction['reference_type'] == 'sale' ? 'selected' : ''; ?>>판매</option>
                                    <option value="prize" <?php echo $transaction['reference_type'] == 'prize' ? 'selected' : ''; ?>>당첨금</option>
                                    <option value="refund" <?php echo $transaction['reference_type'] == 'refund' ? 'selected' : ''; ?>>환불</option>
                                    <option value="commission" <?php echo $transaction['reference_type'] == 'commission' ? 'selected' : ''; ?>>수수료</option>
                                    <option value="expense" <?php echo $transaction['reference_type'] == 'expense' ? 'selected' : ''; ?>>비용</option>
                                    <option value="other" <?php echo $transaction['reference_type'] == 'other' ? 'selected' : ''; ?>>기타</option>
                                </select>
                            </div>
                            
                            <div class="form-group">
                                <label for="reference_id">참조 ID</label>
                                <input type="text" class="form-control" id="reference_id" name="reference_id" placeholder="관련 참조 ID" value="<?php echo htmlspecialchars($transaction['reference_id'] ?? ''); ?>">
                            </div>
                            
                            <div class="form-group">
                                <label for="notes">비고</label>
                                <textarea class="form-control" id="notes" name="notes" rows="2" placeholder="추가 참고사항"><?php echo htmlspecialchars($transaction['notes'] ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>
