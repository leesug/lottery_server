...
if (function_exists('check_permission')) {
    ...
    error_log("check_permission 함수를 찾을 수 없습니다. (equipment-add.php)");
}

// 판매점 ID 처리 (장비를 특정 판매점에 바로 할당하기 위한 것)
...
$selected_store = null;
if ($store_id > 0) {
    $store_query = "SELECT id, store_name, store_code FROM stores WHERE id = ? AND status = 'active'";
    ...
    if ($stmt) {
        ...
        $selected_store = $result->fetch_assoc();
        $stmt->close();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($serial_number)) {
        ...
        // 시리얼 번호 중복 검사
        $serial_check_query = "SELECT id FROM store_equipment WHERE serial_number = ?";
    ...
    if (empty($errors)) {
        ...
        try {
            // 장비 코드 생성
            $equipment_code = generate_equipment_code($equipment_type);
            ...
            if (!$insert_result) {
                throw new Exception("장비 정보 저장 실패: " . $conn->error);
            }
            
            // 정기 점검 일정 생성 (설치일로부터 3개월 후)
            $next_maintenance_query = "
                UPDATE store_equipment 
                SET next_maintenance_date = DATE_ADD(?, INTERVAL 3 MONTH)
                WHERE id = ?
            ";
            
            $maintenance_stmt = $conn->prepare($next_maintenance_query);
            $maintenance_stmt->bind_param("si", $installation_date, $new_equipment_id);
            $maintenance_result = $maintenance_stmt->execute();
            $maintenance_stmt->close();
            
            if (!$maintenance_result) {
                throw new Exception("정기 점검 일정 생성 실패: " . $conn->error);
            }
            ...
            $log_query = "
                INSERT INTO activity_logs (
                    ...
                    ip_address, 
                    user_agent
                ) VALUES (?, 'equipment_add', ?, ?, ?)
            ";
            
            $log_description = "새 {$equipment_type} 장비 등록 (모델: {$equipment_model}, SN: {$serial_number})";
            ...
            // 성공 메시지 설정
            set_flash_message('success', '새 장비가 성공적으로 등록되었습니다.');
            
            // 장비 상세 페이지로 리다이렉트
            redirect_to("/dashboard/store/equipment-details.php?id={$new_equipment_id}");
            exit;
            ...
            // 오류 로그 기록
            log_error("장비 등록 실패: " . $e->getMessage());
            
            // 오류 메시지 설정
            $errors[] = "장비 등록 중 오류가 발생했습니다: " . $e->getMessage();
        }
    }
}
?>
...
<div class="container-fluid px-4">
    ...
    <ol class="breadcrumb mb-4">
        ...
        <li class="breadcrumb-item active">새 장비 등록</li>
    </ol>
            ...
            <form method="post" action="" class="needs-validation" novalidate>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="equipment_model" id="equipment_model" class="form-control" required>
                        <div class="invalid-feedback">
                            장비 모델을 입력해주세요.
                        </div>
                    </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="installation_date" id="installation_date" class="form-control" required value="<?php echo date('Y-m-d'); ?>">
                        <div class="invalid-feedback">
                            설치일을 입력해주세요.
                        </div>
                    </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="maintenance_notes" id="maintenance_notes" class="form-control" rows="3"></textarea>
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    ...
                    <button type="submit" class="btn btn-primary">장비 등록</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 초기 실행 (페이지 로드 시)
    if (installationDateInput.value) {
        const event = new Event('change');
        installationDateInput.dispatchEvent(event);
    }
});
</script>
